<template>
  <div class="continue-chart-container">
	...
	<Stationcoverchart :title="allstationcover.title" :xData="allstationcover.xdata" :yDataone="allstationcover.ydataone" :yDatatwo="allstationcover.ydatatwo" /><!--动态绑定子组件中的props中的值到父组件实例property中的有返回值的值,父组件中组件实例property中对应值的改变，实际数值会传递到子组件中使用props里面的值的地方-->
    </el-row>

    <el-row style="background:#fff;padding:16px 16px 0;margin-bottom:32px;"><!--第一个图表组件-->
	<Stationcoverchart :title="zonestationcover.title" :xData="zonestationcover.xdata" :yDataone="zonestationcover.ydataone" :yDatatwo="zonestationcover.ydatatwo" />
    </el-row>
  </div>
</template>
<script>
