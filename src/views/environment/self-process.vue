<template>
	<div class="continue-chart-container">
		...
		<el-row  style="background:#fff;padding:16px 16px 0;margin-bottom:32px;"><!--第一个图表组件-->
			<SelfProcessChart :chartData="chartDataone.ydata" :xData="chartDataone.xdata" />	
		</el-row>

		<el-row  style="background:#fff;padding:16px 16px 0;margin-bottom:32px;"><!--第一个图表组件-->
			<ZoneSelfProcessChart :chartData="chartDatatwo.ydata" :xData="chartDatatwo.xdata" />
		</el-row>
	</div>
</template>
<script>
...
import SelfProcessChart from './components/SelfProcessChart'
import ZoneSelfProcessChart from'./components/ZoneSelfProcessChart.vue'

export default {
	components: {
		SelectMonth,
		SelfProcessChart,
		ZoneSelfProcessChart	
	},
	data(){
		return{
			chartDataone:{
